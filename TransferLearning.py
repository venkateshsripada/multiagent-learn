...
class TransferLearning:
    ...
    def __init__(self, input_neural_net, input_grid_dimension, output_grid_dimension):
        ...
        self.network_robot_count = (input_neural_net.linear1.in_features -
        ...
        self.output_neural_net = Qnet(self.output_neural_net_state_size, input_neural_net.linear1.out_features)

    # This function is called to perform transfer learning and return the output neural network
    def transfer_learning(self, max_iterations, accepted_mean_square_error=0.01, batch_size=1000, learning_rate=1e-4):
        ...
        for _ in range(0, max_iterations):
            ...
            for batch in range(0, batch_size):
                ...
                for i in range(0, (self.network_robot_count+1)*2, 2):
                    input_network_state[i] = randint(0, input_grid_dimension[0]-1)
                    input_network_state[i+1] = randint(0, input_grid_dimension[1]-1)
                ...
                for i in range(0, row_difference):
                    for j in range(0, column_difference):
                        ...
                        input_network_state.append(np_ndarray.flatten(input_network_known_state))

                        # Now we flatten data in the output network state, a 2-D matrix to 1-D and append
                        output_network_state.append(np_ndarray.flatten(output_network_known_state))
                        ...
                        for k in range(0, 4):
                            ...
                            input_network_state_complete = np_append(input_network_state, k)
                            # Adding an action completes the input state for the output neural network
                            output_network_state_complete = np_append(output_network_state, k)
                            # Getting the Q value predicted by the input neural network for the given state
                            input_network_predicted_value = self.input_neural_net.forward(input_network_state_complete)
                            ...
                            training_list.append([output_network_state_complete, input_network_predicted_value])
            ...
            for batch in range(0, batch_size):
                ...
                output_network_state_complete = training_list[batch][0]
                ...
                output_network_predicted_value = self.output_neural_net.forward(output_network_state_complete)
                ...
                training_error = torch.nn.MSELoss(output_network_predicted_value, predicted_target_value)
